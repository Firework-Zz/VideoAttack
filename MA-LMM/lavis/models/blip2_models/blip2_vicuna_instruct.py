...
class Blip2VicunaInstruct_MALMM(Blip2Base):
    ...
    def generate(
        ...
        elif image.dim() == 4 and image.shape[-1] == 1408:
            is_video = True
            is_feature = True
            B, T, N, C = image.shape
        ...
        if self.qformer_text_input:
            ...
            if self.use_memory_bank:
                ...
                for t in range(T):
                    with self.maybe_autocast():
                        if is_feature:
                            current_feat = image[:, t, :, :].to(self.device)
                            image_embeds = self.ln_vision(current_feat) 
                        else:
                            image_embeds = self.ln_vision(self.visual_encoder(image[:, :, t, :, :]))
        
                        image_embeds = image_embeds.detach() # 保持原有的 detach
                    N, C = image_embeds.shape[-2:]
                    position_ids = torch.tensor([t]).long().to(image_embeds.device)  # [1]
                    position_ids = position_ids.unsqueeze(0).expand(B, -1)  # [B, 1]
                ...
                if is_feature:
                    # image: [B, T, N, C] -> reshape -> [B*T, N, C]
                    image_embeds_input = image.reshape(B * T, N, C).to(self.device)
                    with self.maybe_autocast():
                        image_embeds = self.ln_vision(image_embeds_input)
                else:
                    if is_video:
                        image = image.permute(0, 2, 1, 3, 4).reshape(B * T, C, H, W)
                    with self.maybe_autocast():
