...
def build_videoqa_dataset(dataset_name, split, args, tokenizer):
    if dataset_name == "msvd":
        ...
        import os
        real_csv_root = "/data2/codefile/fjh/just-ask/MSRVTT-QA"
        
        # 1. 确定 CSV
        if split == "train":
            csv_path = os.path.join(real_csv_root, "train.csv")
        elif split == "val":
            csv_path = os.path.join(real_csv_root, "val.csv")
        elif split == "test":
            csv_path = os.path.join(real_csv_root, "test.csv")
        else:
            raise NotImplementedError
            
        # 2. [强制修改] 无论 args 传什么，这里强制指向对抗特征
        # 请确保这个路径是你刚刚合并生成的那个包含 Test 视频的 .pth 文件
        # 如果你跑了全量攻击，用 FULL.pth；如果只跑了 test，用 msrvtt_adv_clip.pth
        features_path = "/data2/codefile/fjh/FrozenBilm/attack_results/clip_attack_no_norm/msrvtt_adv_clip.pth"
        
        # 3. [调试打印] 打印出来看看它到底读了什么，文件多大
        print("!"*60)
        print(f"!!! DEBUG: FORCING MSRVTT FEATURES PATH: {features_path}")
        if os.path.exists(features_path):
            print(f"!!! DEBUG: File exists. Size: {os.path.getsize(features_path) / 1024 / 1024:.2f} MB")
        else:
            print(f"!!! DEBUG: ERROR! File does not exist!")
        print("!"*60)

        # 4. 其他配置
        vocab_path = os.path.join(real_csv_root, "vocab.json")
        type_map = {0: "what", 1: "how", 2: "color", 3: "where", 4: "who", 5: "when"}
        subtitles_path = None
