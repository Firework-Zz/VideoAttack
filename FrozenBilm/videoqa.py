...
import datetime
# ---------------------------------------------------------------------------
# [修改提示] 如果 util.dist 依然报错缺少 hostlist，请确保你已经 pip install hostlist
# 或者去 util/dist.py 把 import hostlist 注释掉
# ---------------------------------------------------------------------------
...
def main(args):
    ...
    if dist.is_main_process():
        ...
        print("Model Name (Tokenizer Path):", args.model_name)
        print("Data/Feature Path (Hardcoded):", args.features_path)
        print("Running in Evaluation Mode:", args.eval)
    ...
    tuples = []
    # 这里的 combine_datasets_val 应该就是 msrvtt
    ...
    for i, item in enumerate(tuples):
        ...
        # -----------------------------------------------------------------------
        # [最终评估] 无论是训练结束，还是直接 Evaluation 模式，这里都会执行
        # -----------------------------------------------------------------------
        if dist.is_main_process():
...
if __name__ == "__main__":
    ...
    # 1. 设置本地 Tokenizer 路径
    args.model_name = "/data0/pretrained/deberta-v2-xlarge/" 
    
    # 2. 【关键】设置数据路径
    args.features_path = "/data2/codefile/fjh/FrozenBilm/attack_results/clip_attack_no_norm/msrvtt_adv_clip.pth"
    args.data_path = "/data2/codefile/fjh/just-ask" 

    # ----------------------------------------------------------------------
    # [FIX 2] 强制开启 Eval 模式以复现结果
    # ----------------------------------------------------------------------
    if args.load and not args.resume:
        print("Detected pretrained weight loading without resume flag. Forcing EVAL mode to reproduce results.")
        args.eval = True
