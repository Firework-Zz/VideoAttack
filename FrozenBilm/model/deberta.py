...
class DebertaV2Embeddings(nn.Module):
    ...
    def forward(
        ...
        if inputs_embeds is None:
            ...
            # [修改点 2] 处理视频特征注入逻辑
            if self.features_dim:
                # 优先使用注入的 video_embeds (用于攻击)
                if video_embeds is not None:
                    inputs_embeds = torch.cat([video_embeds, inputs_embeds], 1)
...
class DebertaV2Model(DebertaV2PreTrainedModel):
    ...
    def forward(
        ...
        # [修改点 4] 更新 Mask 生成逻辑以支持 video_embeds
        if self.features_dim and (video is not None or video_embeds is not None):
            if video_mask is None:
                if video_embeds is not None:
                    # [B, T, D] -> [B, T]
                    video_shape = video_embeds.shape[:-1]
                else:
        ...
        embedding_output = self.embeddings(
            ...
            video=video,
            video_embeds=video_embeds, # [修改点 5] 向下传递
        )
...
class DebertaV2ForMaskedLM(DebertaV2PreTrainedModel):
    ...
    def forward(
        ...
        outputs = self.deberta(
            ...
            video_embeds=video_embeds, # [修改点 7] 向下传递
        )

        if labels is not None:
            # [修改点 8] 更新标签处理逻辑，支持 video_embeds
            if self.features_dim and (video is not None or video_embeds is not None):
                # ignore the label predictions for visual tokens
                if video_embeds is not None:
                    video_len = video_embeds.shape[1]
