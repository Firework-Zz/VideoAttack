...
class Blip2VicunaInstruct_MALMM(Blip2Base):
    ...
    def forward(self, samples):
        ...
        # ====================================================================================
        # [MODIFIED] Check for adversarial features override
        # ====================================================================================
        if "adv_inputs_llm" in samples:
            # adv_feats shape: [B, 1, T, 32, 4096] if from dataloader collate
            # or [B, T, 32, 4096]
            adv_feats = samples["adv_inputs_llm"].to(image.device)
            if adv_feats.dim() == 5 and adv_feats.size(1) == 1:
                adv_feats = adv_feats.squeeze(1) # [B, T, 32, 4096]
            
            # Reshape to [B, T*32, 4096] to match inputs_llm expectation
            inputs_llm = adv_feats.reshape(adv_feats.size(0), -1, adv_feats.size(-1))
            inputs_llm = inputs_llm.to(self.llm_model.dtype)
            
            # Bypass visual encoder and Qformer
            # Note: inputs_llm here is already in LLM hidden space, no llm_proj needed
            
        else:
            # --- Original Visual Encoding Logic ---
            if self.qformer_text_input:
                ...
                if self.use_memory_bank:
                    ...
                    for t in range(T):
                        ...
                        if t == 0:
                            self.visual_memory_bank = image_embeds.detach()
                            self.compression_size = self.size_constant
                        else:
                            self.visual_memory_bank = torch.cat([self.visual_memory_bank, image_embeds.detach()], dim=1)
    ...
    def generate(
        ...
        # ====================================================================================
        # [MODIFIED] Check for adversarial features override in generate
        # ====================================================================================
        if "adv_inputs_llm" in samples:
            adv_feats = samples["adv_inputs_llm"].to(image.device)
            if adv_feats.dim() == 5 and adv_feats.size(1) == 1:
            ...
            inputs_llm = adv_feats.reshape(adv_feats.size(0), -1, adv_feats.size(-1))
            inputs_llm = inputs_llm.to(self.llm_model.dtype)
            # Skip visual/qformer blocks

        else:
            # --- Original Visual Processing ---
    ...
    def predict_class(
        ...
        if type(candidates[0]) == list:
            ...
            for i in range(samples["image"].size(0)):
                ...
                # Copy adv features for single sample if present
                if "adv_inputs_llm" in samples:
                    this_sample["adv_inputs_llm"] = samples["adv_inputs_llm"][i].unsqueeze(0)
    ...
    def _predict_class(
        ...
        # ====================================================================================
        # [MODIFIED] Inject adversarial features directly
        # ====================================================================================
        if "adv_inputs_llm" in samples:
            # adv_inputs_llm: [B, T, 32, 4096] (or [B, 1, T, 32, 4096] depending on collate)
            adv_feats = samples["adv_inputs_llm"].to(image.device)
            if adv_feats.dim() == 5 and adv_feats.size(1) == 1:
                adv_feats = adv_feats.squeeze(1)
            
            # Dimensions
            B_ext, T_ext, Q, D = adv_feats.shape
            assert B_ext == bs, f"adv_inputs_llm batch size {B_ext} != image batch size {bs}"
            
            # Flatten to [B, T*Q, D]
            inputs_llm = adv_feats.reshape(bs, T_ext * Q, D)
            inputs_llm = inputs_llm.to(self.llm_model.dtype)
            atts_llm = torch.ones(inputs_llm.size()[:-1], dtype=torch.long).to(image.device)
